...
class GoogleClient extends BaseClient {
  ...
  setOptions(options) {
    ...
    const normalizedInstructions = this.normalizeInstructionsPayload(promptPrefix, {
      logPrefix: '[GoogleClient][setOptions]',
      encodingResolver: () => this.getEncoding(),
    });
    this.systemInstructions = normalizedInstructions;
    this.systemMessage = normalizedInstructions.content;
  ...
  async buildAugmentedPrompt(messages = []) {
    ...
    if (this.contextHandlers) {
      ...
      this.systemInstructions = this.normalizeInstructionsPayload(this.systemMessage, {
        logPrefix: '[GoogleClient][buildAugmentedPrompt]',
        encodingResolver: () => this.getEncoding(),
      });
      this.systemMessage = this.systemInstructions.content;
    }
  }
  ...
  async buildMessages(_messages = [], parentMessageId) {
    ...
    if (this.systemMessage) {
      const normalizedInstructions =
        this.systemInstructions?.content === this.systemMessage
          ? this.systemInstructions
          : this.normalizeInstructionsPayload(this.systemMessage, {
              logPrefix: '[GoogleClient][buildMessages]',
              encodingResolver: () => this.getEncoding(),
            });

      this.systemInstructions = normalizedInstructions;

      const instructionsTokenCount = normalizedInstructions.tokenCount;
  ...
  async getCompletion(_payload, options = {}) {
    ...
    try {
      if (!EXCLUDED_GENAI_MODELS.test(modelName) && !this.project_id) {
        ...
        const instructionsContent = (this.systemInstructions?.content ?? '').trim();
        ...
        const result = await this.runWithClientResilience(
          async () =>
            client.generateContentStream(requestOptions, {
  ...
  async titleChatCompletion(_payload, options = {}) {
    ...
    if (!EXCLUDED_GENAI_MODELS.test(model) && !this.project_id) {
      ...
      const titleResponse = await this.runWithClientResilience(
        async () =>
