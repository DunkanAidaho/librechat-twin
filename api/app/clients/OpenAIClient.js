...
class OpenAIClient extends BaseClient {
  ...
  async buildMessages(messages, parentMessageId, { promptPrefix = null }, opts) {
    ...
    if (promptPrefix && !noSystemModelRegex) {
      ...
      const normalizedInstructions = this.normalizeInstructionsPayload(instructionsText, {
        logPrefix: '[OpenAIClient][buildMessages]',
        encodingResolver: () => this.getEncoding(),
      });

      promptPrefix = instructionsText;

      if (normalizedInstructions.content) {
        instructions = {
          role: 'system',
          content: normalizedInstructions.content,
        };

        if (this.contextStrategy) {
          instructions.tokenCount = normalizedInstructions.tokenCount;
        }
      }
    }
