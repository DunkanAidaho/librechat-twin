...
if (!Conversation.schema.path('lastSummarizedIndex')) {
  Conversation.schema.add({ lastSummarizedIndex: { type: Number, default: 0 } });
  try {
    if (typeof Conversation.syncIndexes === 'function') {
      Conversation.syncIndexes().catch((err) =>
        logger.warn('[Conversation] Не удалось синхронизировать индексы:', err?.message || err),
      );
    }
  } catch (err) {
    logger.warn('[Conversation] Ошибка при добавлении поля lastSummarizedIndex:', err?.message || err);
  }
}
