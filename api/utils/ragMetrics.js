...
const segmentCounter = reuseMetric('rag_tokens_total', () =>
  new Counter({
...
const cacheCounter = reuseMetric('rag_cache_hits_total', () =>
  new Counter({
...
const costCounter = reuseMetric('rag_request_cost_total', () =>
  new Counter({
...
const ragContextGauge = reuseMetric('rag_context_length_chars', () =>
  new Gauge({
...
function setContextLength(type, length) {
  if (!type || length == null) {
    return;
  }
  ragContextGauge.labels(type).set(length);
}
