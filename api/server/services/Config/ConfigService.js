...
const ragProvidersSchema = z.object({
...
class ConfigService {
    ...
    return {
      ...
      rag: {
        ...
        loader: () => {
          ...
          if (!toolsGatewayUrl) {
            logger.warn('[ConfigService] rag.gateway.url is not configured; tools-gateway features are disabled.');
          }
          const ragCacheTtl = parseOptionalInt(this.env.RAG_CACHE_TTL) ?? 900;
          const toolsGatewayTimeout = parseOptionalInt(this.env.TOOLS_GATEWAY_TIMEOUT_MS) ?? 20_000;
          const fallbackGraphLines = parseOptionalInt(this.env.RAG_GRAPH_MAX_LINES);
          const ragGraphMaxLines = parseOptionalInt(this.env.GRAPH_CONTEXT_LINE_LIMIT) ?? fallbackGraphLines ?? 8;
          const ragGraphMaxLineChars = parseOptionalInt(this.env.GRAPH_CONTEXT_MAX_LINE_CHARS) ?? 200;
          const ragGraphSummaryLineLimit = parseOptionalInt(this.env.GRAPH_CONTEXT_SUMMARY_LINE_LIMIT) ?? Math.min(ragGraphMaxLines, 8);
          const ragGraphSummaryHintMaxChars = parseOptionalInt(this.env.GRAPH_CONTEXT_SUMMARY_HINT_MAX_CHARS) ?? 2_000;
          ...
          const ragVectorEmbedding = sanitizeOptionalString(this.env.RAG_SEARCH_MODEL) || 'mxbai';
          ...
          const ragSummaryEnabled = parseOptionalBool(this.env.RAG_SUMMARIZE_IF_OVER) ?? true;
          const ragSummaryProvider = sanitizeOptionalString(this.env.RAG_VECTOR_SUMMARY_PROVIDER);
          const ragQueryMaxChars = parseOptionalInt(this.env.RAG_QUERY_MAX_CHARS) ?? 6_000;
          const historyHistLongUser = parseOptionalInt(this.env.HIST_LONG_USER_TO_RAG) ?? 20_000;
          const historyOcrThreshold = parseOptionalInt(this.env.OCR_TO_RAG_THRESHOLD) ?? 15_000;
          const historyWaitForIngest = parseOptionalInt(this.env.WAIT_FOR_RAG_INGEST_MS) ?? 0;
          const historyAssistLong = parseOptionalInt(this.env.ASSIST_LONG_TO_RAG) ?? 15_000;
          const historyAssistSnippet = parseOptionalInt(this.env.ASSIST_SNIPPET_CHARS) ?? 1_500;
          return {
            ...
            context: {
              ...
              summaryHintMaxChars: parseOptionalInt(this.env.GRAPH_CONTEXT_SUMMARY_HINT_MAX_CHARS) ?? 1_000,
              includeGraphInSummary: parseOptionalBool(this.env.GRAPH_CONTEXT_INCLUDE_IN_SUMMARY) ?? true
            },
            condense: {
            ...
            providers: {
              ...
              openrouter: {
                ...
                referer: sanitizeOptionalString(this.env.OPENROUTER_REFERRER || this.env.SERVER_DOMAIN || this.env.PUBLIC_SERVER_DOMAIN),
                appName: sanitizeOptionalString(this.env.OPENROUTER_APP_NAME)
              },
              ollama: {
                url: sanitizeUrl(this.env.OLLAMA_SUMMARIZATION_URL || this.env.OLLAMA_URL || undefined) || undefined,
      ...
      pricing: {
        ...
        loader: () => {
          ...
          const baseUrl = sanitizeOptionalString(this.env.OPENROUTER_BASE_URL) || 'https://openrouter.ai/api/v1';
          ...
          return {
            ...
            cachePath: sanitizeOptionalString(this.env.OPENROUTER_PRICING_CACHE_PATH) || './api/cache/openrouter_pricing.json'
          };
        }
      },
      security: {
      ...
      features: {
        ...
        loader: () => ({
          ...
          googleChainBuffer: (sanitizeOptionalString(this.env.GOOGLE_CHAIN_BUFFER) || 'off').toLowerCase(),
      ...
      memory: {
        schema: memorySchema,
        loader: () => {
          const useConversationMemory = parseOptionalBool(this.env.USE_CONVERSATION_MEMORY) ?? true;
          const enableMemoryCache = parseOptionalBool(this.env.ENABLE_MEMORY_CACHE);
          const fallbackGraphLines = parseOptionalInt(this.env.RAG_GRAPH_MAX_LINES);
          const graphMaxLines = parseOptionalInt(this.env.GRAPH_CONTEXT_LINE_LIMIT) ?? fallbackGraphLines ?? 8;
          const graphSummaryLineLimit = parseOptionalInt(this.env.GRAPH_CONTEXT_SUMMARY_LINE_LIMIT) ?? Math.min(graphMaxLines, 8);
          const graphSummaryHintMaxChars = parseOptionalInt(this.env.GRAPH_CONTEXT_SUMMARY_HINT_MAX_CHARS) ?? 2_000;
          const graphMaxLineChars = parseOptionalInt(this.env.GRAPH_CONTEXT_MAX_LINE_CHARS) ?? 200;
          const graphRequestTimeoutMs = parseOptionalInt(this.env.GRAPH_REQUEST_TIMEOUT_MS) ?? 10_000;
          const ragQueryMaxChars = parseOptionalInt(this.env.RAG_QUERY_MAX_CHARS) ?? 6_000;
          return {
            temporalEnabled: parseOptionalBool(this.env.TEMPORAL_MEMORY_ENABLED) ?? false,
            graphWorkflowEnabled: parseOptionalBool(this.env.MEMORY_GRAPHWORKFLOW_ENABLED) ?? false,
            ...
            queue: {
              ...
              historySyncBatchSize: parseOptionalInt(this.env.HISTORY_SYNC_BATCH_SIZE) ?? 20
            },
            graphContext: {
      ...
      logging: {
        ...
        loader: () => {
          ...
          const resolvedMode = normalizedMode && TOKEN_USAGE_REPORT_MODES.includes(normalizedMode) ? normalizedMode : 'json';
          const tokenBreakdownEnabled = parseOptionalBool(this.env.TOKEN_BREAKDOWN_ENABLED);
          const tokenBreakdownLevel = sanitizeOptionalString(this.env.TOKEN_BREAKDOWN_LEVEL) || "info";
          return {
            branch: {
            ...
            tokenBreakdown: {
              enabled: tokenBreakdownEnabled ?? true,
        ...
        loader: () => {
          ...
          return {
            ...
            token: {
              ...
              truncateLongMessages: parseOptionalBool(this.env.TRUNCATE_LONG_MESSAGES) ?? true
            },
            maxUserMsgToModelChars: parseOptionalInt(this.env.MAX_USER_MSG_TO_MODEL_CHARS) ?? 0,
      ...
      agents: {
        ...
        loader: () => ({
          resilience: {
            ...
            maxDelayMs: parseOptionalInt(this.env.AGENT_RETRY_MAX_DELAY_MS) ?? Math.max(200, parseOptionalInt(this.env.AGENT_RETRY_MIN_DELAY_MS) ?? 200),
          ...
          thresholds: {
            ...
            googleNoStreamThreshold: parseOptionalInt(this.env.GOOGLE_NOSTREAM_THRESHOLD) ?? 120_000
          },
          encoding: {
            defaultTokenizerEncoding: sanitizeOptionalString(this.env.DEFAULT_TOKENIZER_ENCODING) || 'o200k_base'
          },
          titles: {
  ...
  #loadSection(name, options = {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        logger.error('[ConfigService] Validation error in section %s: %o', name, error.flatten().fieldErrors);
      } else {
   ...
   * Возвращает настройки логирования токенизации с учётом ENV.
   * @returns {{ enabled: boolean, detailLevel: string, includePrompt: boolean, includeMessages: boolean }}
   */getTokenizationLogSettings() {const rawLevel = sanitizeOptionalString(this.env.TOKENIZATION_DETAIL_LEVEL);const normalizedLevel = rawLevel ? rawLevel.toLowerCase() : undefined;const validLevels = ['basic', 'detailed', 'full'];const detailLevel = validLevels.includes(normalizedLevel) ? normalizedLevel : 'basic';const enabled = parseOptionalBool(this.env.DEBUG_TOKENIZATION) ?? false;const includePrompt = parseOptionalBool(this.env.LOG_TOKENIZATION_PROMPT) ?? false;const includeMessages = parseOptionalBool(this.env.LOG_TOKENIZATION_MESSAGES) ?? true;return { enabled: enabled, detailLevel: detailLevel, includePrompt: includePrompt, includeMessages: includeMessages };}getSection(name) {return this.#loadSection(name);}
  ...
  #useDefault(path, defaultValue) {
    if (!this.missingDefaults.has(path)) {
      ...
      logger.warn(`[ConfigService] Value for "${path}" is missing; falling back to default (${JSON.stringify(defaultValue)})`);
    }
    return defaultValue;
  }
  #assertCritical() {
    ...
    if (!queues.toolsGatewayUrl) {
      logger.warn('[ConfigService] toolsGatewayUrl is not configured. Temporal HTTP fallback may be unavailable.');
    }
