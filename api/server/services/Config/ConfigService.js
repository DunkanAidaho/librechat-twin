...
const limitsSchema = z.object({
    ...
    const memorySchema = z.object({
      ...
      queue: z.object({
        ...
        enqueueBatchMin: z.number().int().positive().optional(),
        enqueueBatchMax: z.number().int().positive().optional(),
    ...
    if (diag && ragConfig.summarization) {
      ...
      queue: {
        ...
        enqueueBatchMin: parseOptionalInt(this.env.MEMORY_QUEUE_BATCH_MIN) ?? 5,
        enqueueBatchMax: parseOptionalInt(this.env.MEMORY_QUEUE_BATCH_MAX) ?? 25,
        ...
        backoffMaxMs: parseOptionalInt(this.env.MEMORY_QUEUE_BACKOFF_MAX_MS) ?? 5000,
      ...
      try {
        ...
        if (fileConfig && typeof fileConfig === 'object') {
          if (fileConfig.endpoints) {
            ...
            logger.info(
              `[ConfigService] Успешно загружена секция 'endpoints' из YAML: ${configPath}`,
            );
          }
          ...
          if (fileConfig.fileConfig) {
            ...
            logger.info(
              `[ConfigService] Успешно загружена секция 'fileConfig' из YAML: ${configPath}`,
            );
          }
        }
      } catch (e) {
        logger.error(
          `[ConfigService] Не удалось загрузить или распарсить YAML конфиг по пути ${configPath}`,
          e,
        );
      }
    }
