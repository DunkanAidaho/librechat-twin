// /opt/open-webui/manage_summaries.js - скользящее окно с перекрытием, RPUSH в summarization_queue
...
const {
  ...
  REDIS_HOST,
  REDIS_PORT,
  REDIS_PASSWORD, // <-- ИСПРАВЛЕНО: Добавлено сюда
  REDIS_SUMMARIZATION_QUEUE_NAME,
  SUMMARY_OVERLAP, // новое: перекрытие окна
} = process.env;
...
const OVERLAP = Math.max(0, parseInt(SUMMARY_OVERLAP || '5', 10)); // по умолчанию 5
const BATCH_SIZE = parseInt(process.env.SUMMARY_PUBLISH_BATCH || '20', 10);
...
function getRedisClient() {
  // ИЗМЕНЕНО: Добавлен password
  const client = new Redis({ host: REDIS_HOST, port: REDIS_PORT, password: REDIS_PASSWORD, maxRetriesPerRequest: 3 });
  client.on('error', (err) => logger.error('[MANAGE_SUMMARIES] Redis client error', err));
  return client;
}

async function publishToRedis(redis, tasks) {
  if (!redis || !REDIS_SUMMARIZATION_QUEUE_NAME) {
    logger.error('[MANAGE_SUMMARIES] Redis client or queue name not available.');
    return 0;
  }
  try {
    const stringified = tasks.map((task) => JSON.stringify(task));
    // батчами по BATCH_SIZE
    for (let i = 0; i < stringified.length; i += BATCH_SIZE) {
      const batch = stringified.slice(i, i + BATCH_SIZE);
      await redis.rpush(REDIS_SUMMARIZATION_QUEUE_NAME, ...batch);
    }
    logger.info(`[MANAGE_SUMMARIES] Published ${tasks.length} task(s) to queue: ${REDIS_SUMMARIZATION_QUEUE_NAME}.`);
    return tasks.length;
  } catch (err) {
    logger.error('[MANAGE_SUMMARIES] Error publishing tasks to Redis:', err);
    return 0;
  }
}
...
async function handleResummarize(filter, redis) {
  const targetDescription = filter.conversationId ? `conversation ${filter.conversationId}` : 'all conversations';
  ...
  for (const convo of conversations) {
    ...
    while (currentIndex < messageCount) {
      ...
      const publishedCount = await publishToRedis(redis, [task]);
      ...
      // Доп. условие: если оставшихся сообщений меньше порога — выходим
      if (messageCount - currentIndex < SUMMARIZATION_THRESHOLD) {
...
(async () => {
  ...
  try {
    ...
    if (args[0] === 'reset') {
      ...
      await handleResummarize(filter, redis);
    }
  } catch (err) {
