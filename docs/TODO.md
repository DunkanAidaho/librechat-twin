...
| F-2.1 | Иерархический RAG и guardrails до LLM | Критический | 2 | В работе | **Зачем:** исключить «всё подряд» и переполнение до первого запроса. **Что делаем:** (1) **иерархический RAG**: vector → entity extraction по top‑chunks → graph по критичным сущностям → condense; (2) **динамические бюджеты по лимиту модели**, без фиксированных 400 токенов; (3) **guardrails**: жёсткий cap ragBlock, per‑model chunk/budget limits до LLM; (4) **логирование без метрик**: vector hits, entity selection, graph lines, ragBlock size, overflow. **Подробности:** см. обновление декомпозиции. | [`plans/f2_context_decomposition.md`](plans/f2_context_decomposition.md), [`api/server/services/RAG/multiStepOrchestrator.js`](api/server/services/RAG/multiStepOrchestrator.js), [`api/server/services/agents/context/builder.js`](api/server/services/agents/context/builder.js), [`api/server/services/RAG/condense.js`](api/server/services/RAG/condense.js), [`api/server/services/RAG/RagContextManager.js`](api/server/services/RAG/RagContextManager.js) |
| F-2.2 | Интегрировать Temporal SummaryWorkflow в prompt‑summary | Высокий | 3 | Ожидает | **Зачем:** использовать результаты summary воркера при сборке prompt в `BaseClient`, сократив историю без потери смысла. **Что делаем:** 1) определить контракт хранения summary (в `Message.summary` и `summaryTokenCount`); 2) добавить интеграцию чтения summary из tools‑gateway/Temporal (или отдельной коллекции) и запись в `Message`; 3) обеспечить совместимость с `live_window`/`RAG‑condense`; 4) добавить ENV‑переключатель для включения/выключения; 5) добавить fallback при превышении gRPC payload (>4MB) и лимитировать payload/батчи. **Техдолг:** описать протокол, обработку ошибок, повторную индексацию. Минимум 10000 символов описания при реализации. | [`api/server/controllers/agents/request.js`](api/server/controllers/agents/request.js), [`api/app/clients/BaseClient.js`](api/app/clients/BaseClient.js), [`api/server/services/agents/MessageHistoryManager.js`](api/server/services/agents/MessageHistoryManager.js) |
